...
def get_github_repos(
    ...
    if output:
        ...
        click.echo(f"Wrote {len(output_data['repositories'])} repositories to {output_path}", err=True)
    else:
...
def _has_terraform_files(repo: Dict[str, Any], headers: Dict[str, str]) -> bool:
    ...
    try:
        ...
        params = {
            "q": f"extension:tf repo:{repo['full_name']}",
            "per_page": 1
        }
        ...
        if response.status_code == 200:
            ...
            click.echo(f"Warning: Cannot check Terraform files for {repo['name']} (rate limited)", err=True)
